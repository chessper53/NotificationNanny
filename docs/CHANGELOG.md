...
- **A custom banner could show another app's icon, or the bell for every notification.** Since macOS 26 all banners share one window, and the app name was cached per window, so the first banner's app stuck to the ones after it. The name is now read again for every new banner.
- A failed icon lookup was never retried, so an app installed after NotificationNanny started kept the bell until relaunch. It is now retried after 30 seconds.
...
- The release workflow's cask bump finally lands. Every run from 7.3.1 to 7.7.0 failed at its final step: the `release` event checks out a detached HEAD, so a bare `git push` had nothing to push to and `Casks/notificationnanny.rb` was never updated, leaving the bump to be done by hand. It now pushes explicitly to `main`. The zip upload also uses `--clobber`, so the job can be re-run after a later step fails instead of colliding with the asset it already attached.
